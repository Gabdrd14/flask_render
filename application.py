...
@app.route('/categorie/<id_cat>', methods = ['GET'])

def supprimer_categ(id_cat):


    bdd = obtenir_bdd()    
    curseur = bdd.execute(''' SELECT id ,nom, description FROM categories WHERE id = ?''',(id_cat,)) 

    categ = curseur.fetchone()
    # print(categ["nom"])

    if categ['id'] !=  None :
            curseur = bdd.execute(''' DELETE  FROM categories WHERE id = ?''',(categ['id'],)) 
            bdd.commit()
            return  f""" La catégorie est {categ['id']} est delete """

    else :
        return f"""{categ['id']} inexistant  """
